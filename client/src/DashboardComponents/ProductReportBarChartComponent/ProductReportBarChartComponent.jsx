...
function ProductReportBarChartComponent({ heading, line }) {
   ...
   useEffect(() => {
      if (!!productGenralReport && productGenralReport?.success && !!productGenralReport?.report.length) {
         setDataSet({
      ...
      if (!!line && !!productGenralReport && productGenralReport?.success && !!productGenralReport?.report.length) {
         const total = productGenralReport.report.map((el) => el.totalProductUploded).reduce((acc, crv) => acc + crv);
   ...
   return (
      <styled.div>
         <HeadingComponent Heading={!!line && Products.current ? `${heading} ${Products.current}` : heading} />
         {!!DataSet && !!productGenralReport && productGenralReport?.success && productGenralReport?.report ? (
            !!line ? (
               <Line data={DataSet} />
            ) : (
               <Bar data={DataSet} />
